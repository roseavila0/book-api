...
const dataPath = path.join(__dirname, '../data/authors.json');
    ...
    const searchAuthor = (searchInfo) => {
        ...
        const result = authors.find(author =>      //buscamos en el arreglo de autores un autor cuyo nombre o nacionalidad sea igual a la que ingresamos para buscar.
                author.name.toLowerCase()  === searchInfo.toLowerCase() //||
               // author.nationality.toLowerCase() === searchInfo.toLowerCase()
        );
                console.log("Search result in model:", result);  // Usamos un console.log para Depuración, poder ver el resultado de la busqueda.
                return result || { error: "Author not found" }; // nos da l resultado de la busqueda si es que esta correcto todo o si hay errs nos muestra un msj de error.
